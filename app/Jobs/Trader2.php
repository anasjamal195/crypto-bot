...
class Trader2 implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        Log::info('Trader 2: Auto Trade Started.');
        Log::info('Trader 2: Symbol: ' . $this->symbol);
        Log::info('Trader 2: Candle Interval: ' . $this->interval);
        Log::info('Trader 2: Profit: ' . $this->estimated_profit . '%');
        Log::info('Trader 2: Stop Loss Limit: ' . $this->stop_limit);
        Log::info('Trader 2: Timeout Limit: ' . $this->timeout_limit);
        Log::info('Trader 2: Amount: ' . $this->buy_coin_price);
        Log::info('Trader 2: Trade Account: ' . $this->trade_acc);
        ...
        while (true) {
            try {
                ...
                if (!empty($current_open_order_sell) && !empty($current_open_limit_order_buy)) {
                    ...
                    Log::info('Trader 2: Another order is open ' . $current_open_order_sell->orderId);
                    ...
                    if ($current_order_api['status'] == 'FILLED' || $current_order_api['status'] == 'CANCELED') {
                        ...
                        Log::info('Trader 2: Previous Trade Closed ');
                    } else {
                        ...
                        if ($current_price  <= $stop_limit_amount || Carbon::now()->greaterThan($timeoutTimestamp)) {
                            ...
                            //Cancel this order
                            Log::info('Trader 2: Cancelling order ');
                            $cancel_response = cancelOrder($this->symbol, $current_open_order_sell->orderId, $this->trade_acc);
                            Log::info('Trader 2: Cancel response ' . json_encode($cancel_response));
                            // ----------------
                            Log::info('Trader 2: Selling at market price');
                            $sell_response = placeSellMarketOrder($this->symbol, $current_quantity, $current_price, $this->trade_acc);
                            Log::info('Trader 2: MARKET SELL Response = ' . json_encode($sell_response));
                            Log::info('Trader 2: Trade closed due to STOP LOSS');
                        }
						*/
                    }
                    continue;
                    ...
                    if (!empty($current_open_limit_order_buy)) {
						...
						*/
                        ...
                        if ($current_order_api['status'] == 'FILLED') {
                            ...
                            Log::info('Trader 2: Current Open Buy Order ' . $current_open_limit_order_buy->orderId);
                            continue;
                        }
                    }
                ...
                // Log error message
                Log::error('Trader 2: Error occurred while processing ProcessTask job: ' . $e->getMessage() . PHP_EOL . $e->getTraceAsString());
                sendEmailException('Trader 2: Error occurred while processing ProcessTask job: ' . $e->getMessage() . PHP_EOL . $e->getTraceAsString());
                continue;
            }
