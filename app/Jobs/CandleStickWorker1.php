...
class CandleStickWorker1 implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        // Log::info('CandleStickWorker1 Previous records flushed ');
        Log::info('CandleStickWorker1 started for Symbol: ' . $this->symbol);
        Log::info('CandleStickWorker1: Candle Interval: ' . $this->interval);
        Log::info('CandleStickWorker1: BUY Coin Price: ' . $this->buy_coin_price);
        Log::info('CandleStickWorker1: Trade Account: ' . $this->trade_acc);
        ...
        while (true) {
            try {
                ...
                if ($api_response['is_open']) {
                    Log::info('CandleStickWorker1: Open order found for ' . $this->symbol);
                    $this->manageOpenOrder($api_response['order']);
                    continue;
                } else {
                    Log::info('CandleStickWorker1: No open orders found ' . $this->symbol);
                }

                $candleData = getCandleStickDataNew($this->symbol, $this->interval, 100);
                $buyPoints = detectBuyPoint($candleData);
                if (count($buyPoints) == 0) {
                    Log::info('CandleStickWorker1: No Buying point found for coin ' . $this->symbol);
                    continue;
                }
                ...
                if ($isUnderInterval) {
                    Log::info('CandleStickWorker1: Buy conditions met, executing buy order ' . $this->symbol);
                    $this->executeBuy();
                } else {
                    Log::info('CandleStickWorker1: Buy conditions not met ' . $this->symbol);
                }
            } catch (\Exception $e) {
                Log::error('CandleStickWorker1: Error - ' . $e->getMessage());
    ...
    private function checkOpenOrder(): array
    {
        ...
        Log::info('CandleStickWorker1: checkOpenOrder API response: ', $api_response);
    ...
    private function manageOpenOrder(array $buy_order): void
    {
        ...
        Log::info('CandleStickWorker1: Current Price: ' . $current_price);
        Log::info('CandleStickWorker1: Buy Order Price: ' . $buy_order['price']);
        Log::info('CandleStickWorker1: Current Profit: ' . $current_profit . '%');
        ...
        if ($current_profit > 0.3 && $current_price > $this->previous_price) {
            ...
            Log::info('CandleStickWorker1: Updated Stop-Loss Percentage: ' . $this->dynamic_stop_loss_percentage . '%');
            Log::info('CandleStickWorker1: New Stop Loss: ' . $new_stop_loss);
            ...
            Log::info('CandleStickWorker1: Updated Stop Loss: ' . $this->dynamic_stop_loss);
        }else if($this->dynamic_stop_loss == 0){
             // Set initial dynamic stop-loss
             $this->dynamic_stop_loss = $buy_order['price'] * (100 - $this->dynamic_stop_loss_percentage * 0.9) / 100;
             $this->previous_price = $buy_order['price'];
        }

        $this->previous_price = $current_price;

        Log::info('CandleStickWorker1: Dynamic Stop Loss Updated to: ' . $this->dynamic_stop_loss);
        ...
        if ($current_price < $this->dynamic_stop_loss && $isSellingAllowed) {
            Log::info('CandleStickWorker1: Current price below stop-loss, executing sell.');
    ...
    private function executeBuy(): void
    {
        ...
        Log::info('CandleStickWorker1: Executing Buy Order for ' . $this->symbol);
        Log::info('CandleStickWorker1: Current Price: ' . $current_price);
        Log::info('CandleStickWorker1: Target Buy Price: ' . $target_buy_price);
        Log::info('CandleStickWorker1: Quantity to Buy: ' . $quantity);
        ...
        if (isset($response['price'])) {
            ...
            Log::info('CandleStickWorker1: Buy Order Executed. Response: ' . json_encode($response));
            Log::info('CandleStickWorker1: Initial Stop Loss set at: ' . $this->dynamic_stop_loss);
        } else {
            Log::error('CandleStickWorker1: Buy Order Failed. Response: ' . json_encode($response));
        }
    }
    ...
    {
        Log::info('CandleStickWorker1: Executing Sell Order for ' . $this->symbol);
        Log::info('CandleStickWorker1: Quantity to Sell: ' . $quantity);
        Log::info('CandleStickWorker1: Current Price: ' . $current_price);
        ...
        if (isset($response['success']) && $response['success']) {
            Log::info('CandleStickWorker1: Sell Order Executed Successfully.');
            // Reset stop-loss
            $this->dynamic_stop_loss = 0;
            $this->previous_price = 0;
        } else {
            Log::error('CandleStickWorker1: Sell Order Failed. Response: ' . json_encode($response));
        }
    }
}
